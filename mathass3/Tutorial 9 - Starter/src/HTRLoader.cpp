	...
	if (file)
	{
		...
		while (fgets(buffer, MAX_LINE_LENGTH, file) != NULL)
		{
			...
			if (loc)
			{
				...
				if (!processBasePositionSection(file, loc)) // DONE: IMPLEMENT THIS FUNCTION
					break;

				if (!processAnimationSection(file, loc)) //DONE: IMPLEMENT THIS FUNCTION
					break;
			}
		}
...
void HTRLoader::createGameObjects()
{
	...
	for (JointDescriptor jd : jointDescriptors)
	{
