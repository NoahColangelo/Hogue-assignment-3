...
void SkinnedGameObject::initializeSkeletonFromHTR(std::string htrFilePath, std::string skinWeightFilePath, std::shared_ptr<TTK::OBJMesh> bindMesh)
{
	...
	m_pHTRAnimation->loadHTR(htrFilePath); // DONE: IMPLEMENT THIS FUNCTION

	// Create GameObjects from the data
	m_pHTRAnimation->createGameObjects();  // DONE: IMPLEMENT THIS FUNCTION
...
void SkinnedGameObject::update(float dt)
{
	...
	{
		// Loop the animation of we reach the last keyFrame
		...
		{
			// DONE: create localRotation, scale and translation matrices using HTR data

			m_pLocalRotation =
				glm::mat4_cast(m_pJointAnimation->jointBaseRotation *
					m_pJointAnimation->jointRotations[m_pCurrentFrame]);

			m_pLocalPosition = m_pJointAnimation->jointBasePosition +
		...
		if (m_pParent)
		{
			m_pLocalToWorldMatrix = m_pParent->getLocalToWorldMatrix() * m_pLocalToWorldMatrix;
			m_pJointToBindMat = static_cast<SkinnedGameObject*>(m_pParent)->getJointToBindMatrix() * bindSpaceTransform;
		}
		else
		{
			m_pLocalToWorldMatrix = m_pLocalToWorldMatrix;
